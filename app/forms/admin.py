...
class AdminForm(FlaskForm):
    """Blog设置表单类"""
    blog_title = StringField('Blog名称', validators=[DataRequired('Blog名不能为空'),
                                                 Length(max=128, message='Blog名长度不能超过 128 个字符')])
    blog_subtitle = StringField('Blog副标题', validators=[DataRequired('Blog副标题不能为空'),
                                                     Length(max=256, message='Blog副标题长度不能超过 256 个字符')])
